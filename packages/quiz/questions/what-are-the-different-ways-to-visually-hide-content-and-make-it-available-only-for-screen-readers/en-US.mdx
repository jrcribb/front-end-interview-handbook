...
> [!CAUTION] Don't use `width: 0; height: 0`
>
> Using `width: 0; height: 0` is not recommended because search engines might penalize this as malicious intent, such as keyword stuffing.

## Absolute positioning

`position: absolute; left: -99999px` will position an element way outside the screen. However, as per [WebAIM's article](https://webaim.org/techniques/css/invisiblecontent/):

> [!WARNING] Don't hide interactive elements off-screen
>
...
> [!TIP] Use a battle-tested utility class
>
> Combine the approaches above to make sure it works properly in all browsers. Instead of implementing your own way to remove an element from the rendering tree and the a11y tree, use one of the following utilities from mature CSS frameworks, which have been battle-tested on many websites.
