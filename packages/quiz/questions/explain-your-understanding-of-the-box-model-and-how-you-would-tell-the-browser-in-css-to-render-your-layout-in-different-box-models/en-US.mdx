...
- If no `height` is specified, a block element will be as high as the content it contains, plus `padding` (unless there are floats — see [describe floats and how they work](/questions/quiz/describe-floats-and-how-they-work)).
...
> [!NOTE] Margins are not part of the box itself
>
> `margin`s are not counted towards the actual size of the box. They affect the total space the box takes up on the page, but only the space outside the box. The box's area stops at the `border` — it does not extend into the `margin`.
