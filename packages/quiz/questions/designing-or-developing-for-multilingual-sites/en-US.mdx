...
> [!WARNING] Concatenation breaks in languages with different word order
>
> Do not do anything like `"The date today is " + date`. It will break in languages with different word order. Use a template string with parameter substitution for each language instead. For example, compare the following two sentences in English and Chinese: `I will travel on {% date %}` and `我会在{% date %}出发`. Note that the position of the variable is different due to the grammar rules of each language.
