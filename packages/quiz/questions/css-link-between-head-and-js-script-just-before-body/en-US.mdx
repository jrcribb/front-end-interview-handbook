...
> [!TIP] Prefer `defer` over placing `<script>` at the bottom
>
> If you need to put `<script>`s in the `<head>`, use the `defer` attribute. It achieves the same effect of running the script only after the HTML is parsed, but the browser can kick off the network request earlier to download the script.
